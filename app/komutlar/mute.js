...
exports.run = async (client, message, args) => {
  ...
  let süre = args.slice(1).join('gün').replace('gün'.toLowerCase(), 'gün  ').replace('saat'.toLowerCase(), 'saat').replace('dakika'.toLowerCase(), 'dakika').replace('saniye'.toLowerCase(), 's');
...
exports.help = {
  ...
  usage: 'sustur @Kullanıcı Sebep [İsterseniz Süre]',
  kategori: 'yetkili'
};
